...
\begin{align}
	\mathcal{C}(\lambda - \mu) = \frac{\dd \phi(x)}{\dd x} \rVert_{x=\lambda-\mu} = \frac{2c}{c^2 + (\lambda - \mu)^2}.
\end{align}
...
\end{align}
With this we can rewrite the displacement functions \cref{eq:partdisplacement} and \cref{eq:holedisplacement} to be valid for a larger domain.
...
\begin{align}
  \label{eq:39}
  \frac{1}{2\pi} (\pi \mathrm{sgn}(\lambda_h) + \phi(\lambda-\lambda_h)) &= \frac{1}{2\pi} \int_{-\mathrm{sgn}(\lambda_h)\infty} \dd \lambda' \frac{\dd}{\dd\lambda'} \phi(\lambda - \lambda') \\
                                                                         &= - \int_{-\mathrm{sgn}(\lambda_h)\infty} \dd \lambda' \mathcal{C}(\lambda-\lambda')
\end{align}
gives
\begin{align}
  \label{eq:40}
...
For the density operator\todo{Hamiltonian in second quantized form derivation to be found in~\cite{feynman}, around p. 190}
\begin{align}
