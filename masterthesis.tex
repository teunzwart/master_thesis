...
\begin{figure}[tb!]
  ...
  \caption{Size of matrix element magnitudes encountered while summing intermediate states of the XXZ model. The yellow line denotes strictly decreasing magnitude. Note that while in general the size of the form factors decreases, not all large form factors are immediately found. Figure taken from~\cite{Caux2009}.}\label{fig:orderedmagnitudes}
\end{figure}
...
An advantage of Q-learning is that it is off-policy, meaning that the optimal action-value function \(q_*\) is approximated independently of the followed policy during training.
...
The algorithm is shown in \cref{alg:qlearning}.
...
\begin{figure}[ph]
  ...
  \caption{Number of Newton iterations until convergence is reached for \(N=5,10,20\), averaged over 1000 sets of Bethe numbers. The black bars denote standard errors.}\label{fig:iterations}
\end{figure}
...
\begin{figure}[ph]
  ...
  \caption{Time in seconds until convergence is reached for \(N=5,10,20\). We average over 10 runs of 1000 sets of Bethe numbers. The black bars denote standard errors.}\label{fig:convtimes}
\end{figure}
