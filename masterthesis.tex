...
Much work has been put in finding correlation functions for various models, but for many there is as yet no closedform solution or this solution may not even exist.
...
\end{align}
If two states have the same momentum, the matrix element vasishes~\cite{slavnov90_noneq_time_curren_correl_funct}, while if $\{\mu\} = \{\lambda\}$ we have $\mathcal{F}=\frac{N}{L}$.
...
ABACUS can not fully compensate for this non-monotinicty, as is seen in \cref{fig:orderedmagnitudes}, where the absolute magnitude of form factors is plotted as a function of their order in the computation.
