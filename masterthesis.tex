...
The wave function in this case becomes~\cite{Franchini2017}\todo{Bring the presentation of this in line with the two particle case.}
\begin{equation}
...
- Bethe equations\\
- ground state\\
- ground state density (iterative scheme for density)\cite{Zemyan2012}
...
The form factor for $(\Psi^{\dag}(0))^2\Psi(0)^2$ has a closed form determinant solution and is easy to work with because both states under consideration have the same number of particles. 
...
Some of the more high profile examples of this are self-driving cars (being developed by many technology and car companies) \todo{Cite this somehow?} and the development of AlphaGo, which beat the world champion in the Japanese board game of Go by learning from both human professional games and playing against itself~\cite{Silver2017a,Silver2017}.
...
useful for getting better initial guesses for rapidities!\todo{Really include this, it's still interesting.}
...
Also illegal are those moves that place particles too far away from other particles \todo{Is this necessary? Maybe it is interesting to see what the neural net does on its own, since maybe it will come up with good states based on the training set anyway.}
