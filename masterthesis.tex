...
We will briefly introduce this algebraic Bethe ansatz. \cite{Piroli2015}

\subsection{The algebraic Bethe ansatz}

In the algebraic Bethe ansatz we define our operators in terms of fields.
The quantized Hamiltonian of the Lieb-Liniger model is
\begin{align}
  \label{eq:30}
  H = \int_0^L \dd x \left(\partial_x \Psi(x)^{\dag} \partial_x \Psi(x) + c \Psi^{\dag}(x)\Psi^{\dag}(x)\Psi(x)\Psi(x)\right),
\end{align}
...
\end{align}
If two states have the same momentum, the matrix element is 0~\cite{slavnov90_noneq_time_curren_correl_funct}, while if $\mu = \lambda$ we have (suppressing normalization factors) $\matrixel{\mathbf{\mu}}{\hat{\rho}(x)}{\mathbf{\lambda}}=\frac{N}{L}$.
