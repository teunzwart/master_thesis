...
This occurs through the summation of matrix elements in the Lehmann representation of the DSF.\@
...
\addcontentsline{toc}{section}{Popular summary}
...
\addcontentsline{toc}{section}{Populaire samenvatting}
...
\end{gather}
where \(\mathcal{L}^{(F)}_0\qty(\lambda, \lambda')=0\). 
...
Now we look at form factors for the Lieb-Liniger model, which we will require for calculating dynamical structure functions (see \cref{chap:abacus}).
...
We average the number of states necessary over five runs with random initial seed.
...
\begin{figure}[p]
  ...
  \caption{Saturation as a function of order of computation for random state selection without minimizing \(N_{\mathrm{ph}}\) for five runs of DSF calculation.}\label{fig:saturation_rand_True_check_train_False_check_eval_False}
\end{figure}

\begin{figure}[tb!]
  \centering
  \includegraphics[width=0.7\textwidth]{saturation_histories_rand_True_check_train_False_check_eval_True.pdf}
  \caption{Saturation as a function of order of computation for random state selection while minimizing \(N_{\mathrm{ph}}\) for five runs of DSF calculation.}
\end{figure}
...
\begin{figure}[tb!]
  ...
  \caption{Saturation as a function of order of computation for Q-learning state selection while minimizing \(N_{\mathrm{ph}}\) during both training and DSF calculation, for five runs of DSF calculation.}\label{fig:saturation_histories_rand_False_check_train_True_check_eval_True}
\end{figure}
...
\begin{figure}[tb!]
  ...
  \caption{Saturation as a function of order of computation for Q-learning state selection while minimizing \(N_{\mathrm{ph}}\) during training but not during DSF calculation, for five runs of DSF calculation. Note that one of the DSF calculation runs did not converge, reaching $\sim79.4\%$ saturation after 3000 states.}\label{fig:saturation_histories_rand_False_check_train_True_check_eval_False}
\end{figure}
...
\begin{figure}[tb!]
  ...
  \caption{Saturation as a function of order of computation for Q-learning state selection while not minimizing \(N_{\mathrm{ph}}\) during training but with minimization during DSF calculation, for five runs of DSF calculation.}\label{fig:saturation_histories_rand_False_check_train_False_check_eval_True}
\end{figure}
...
\begin{figure}[tb!]
  ...
  \caption{Saturation as a function of order of computation for Q-learning state selection without minimizing \(N_{\mathrm{ph}}\) during either training and DSF calculation, for five runs of DSF calculation.}\label{fig:saturation_histories_rand_False_check_train_False_check_eval_False}
\end{figure}
...
\begin{table}[h]
  ...
  \begin{tabular}{lccrl}
    ...
    Random & --- & Yes & \num{728(131)} & \num{1.31(60)e-07} \\
    & --- & No & 3000\footnote{No convergence occured for any of the five runs after summing state contributions.} & \num{2.79(143)e-16} \\\midrule
    Q-learning & Yes & Yes & \num{545(151)} & \num{6.53(634)e-07}\\
...
\bibitem{slavnov89_calcul_scalar_produc_wave_funct}
N.~A.~Slavnov, \textit{Calculation of Scalar Products of Wave Functions and Form Factors in the Framework of the Alcebraic Bethe Ansatz}, Theor. Math. Phys. \textbf{79}, 502 (1989), \doi{10.1007/bf01016531}.
