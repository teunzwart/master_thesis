...
Much work has gone into finding correlation functions, but for it is not clear that for a given model closed form solutions exist.
...
Here that we may hope to see an improvement.
...
in \cref{chap:bethe_ansatz} we review some of the basics of the Bethe ansatz and its application in the Lieb-Liniger model, which we will use throughout this thesis because it has properties that make it simpler to work with.
...
Since we must fulfill \cref{eq:lieblinigerboundary} the amplitudes must obey the equation
\begin{align}
...
\begin{align}
  	\sum_{j,l}\frac{\partial^2S}{\partial\lambda_j \partial\lambda_l} v_j v_l = \sum_{j=1}^N L v_{j}^2 + \sum_{j>l=1}^{N} \mathcal{C}(\lambda_j-\lambda_m) {(v_j - v_l)}^2 \geq L \sum_{j=1}^N v_j^{2} > 0,
\end{align}
...
Two types of excitations exist: particle-like (type I) excitations in which a new particle outside the Fermi interval is added with momentum \(k > \left|\lambda_F\right|\) and hole-like (type II) excitations, where a particle is removed from the Fermi interval with momentum \(k < \left|\lambda_F\right|\).
...
Expanding the term in brackets gives\footnote{The expansions goes as follows:
  \begin{align*}
    ...
    = \frac{1}{L}(d_j-d_l) \frac{2c}{c^2 + {\qty(\lambda_j^0-\lambda_l^0)}^2}.
  \end{align*}
}
\begin{align}
  \label{eq:27}
...
\begin{align}
  \label{eq:33}
  (1 - \mathcal{C}^{(F)}) * D_p(\lambda, \lambda_p) = -\frac{\theta(\lambda_F-\abs{\lambda})}{2\pi} \qty(\pi\textrm{sgn}(\lambda_p)) + \phi(\lambda-\lambda_p)
\end{align}
...
\end{gather}
where \(\mathcal{L}^{(F)}_0(\lambda, \lambda')\) is 0. 
...
\end{equation}
where, given \(\mathbf{x}^k\), we calculate \(J(\mathbf{x}^k)\) and \( \mathbf{f}(\mathbf{x}^k)\), and then solve the system of linear coupled equations to get \(\Delta\mathbf{x}=\mathbf{x}^{k+1} - \mathbf{x}^k\), which is added to \(\mathbf{x}^k\) to get \(\mathbf{x}^{k+1}\)~\cite{Sueli2003}. 
...
We save the form factors, and the threshold is lowered after a scan. 
...
\begin{align}
  \label{eq:61}
  E(\mathbf{w}) = \frac{1}{2} \sum _{n=1}^N{\qty(\mathbf{y}(\mathbf{x_n}, \mathbf{w})-\mathbf{t}_n)}^2.
\end{align}
...
\begin{align}
  \label{eq:69}
  E_n = \frac{1}{2} \sum_k {\qty(y_{nk}-t_{nk})}^2,
\end{align}
...
Q-learning is also model-free, meaning that it does not know how the environment the model is interacting with changes based on its actions.
...
In a copy of \(Q\) which we denote \(y\), we update the entry corresponding to the taken action, and then update the neural network to better match the state we want to reach.
...
\begin{figure}[ph]
  ...
  \caption{Time in seconds until convergence for \(N=5,10,20\). We average over 10 runs of 1000 sets of Bethe numbers. The black bars denote standard errors.}\label{fig:convtimes}
\end{figure}
...
Because the vector has to have a finite size this representation does force the use of a UV-cutoff above which there are no Bethe numbers and which can not be occupied.
...
For this number of states we can feasibly generate all combinations and calculate the form factors of the associated states.
...
\begin{figure}[tb]
  ...
  \caption{The number of states in each momentum slice for the Lieb-Liniger model for \(N=5\) and \(I_{\max}=12\). Note that we dropped the prefactor \(\frac{2\pi}{L}\) for each entry.}\label{fig:no_of_states}
\end{figure}
...
If we calculate the sum rule saturations for each momentum slice, the saturations go to 0 as \(\frac{L}{2\pi}P\) becomes larger (\cref{fig:saturationsperslice}), falling off especially for \(\abs{\frac{L}{2\pi}P}>I_{\max}\).
...
We consider six different possible ways of determining states:
\begin{itemize}
...
Now only 728 states are necessary to reach saturation, and average form factor size is of order \(10^{-6}\) (\cref{fig:ff_sizes_rand_True_check_train_False_check_eval_False}).
...
The average form factor also has the largest value at order $10^{-7}$, although the error is large (\cref{fig:ff_sizes_rand_False_check_train_True_check_eval_True}).
...
Here none of the five runs converge (\cref{fig:saturation_histories_rand_False_check_train_False_check_eval_False}) but the average form factor is of order \(10^{-12}\) (\cref{fig:ff_sizes_rand_False_check_train_False_check_eval_False}), meaning learning still occurring.
...
\begin{figure}[tb!]
  ...
  \caption{Square form factors as a function of order of computation for random state selection without minimizing \(N_{\mathrm{ph}}\) for a selected DSF calculation.}\label{fig:ff_sizes_rand_True_check_train_False_check_eval_False}
\end{figure}
...
\begin{figure}[tb!]
  ...
  \caption{Square form factors as a function of order of computation for Q-learning state selection while minimizing \(N_{\mathrm{ph}}\) during both training and DSF calculation, for a selected DSF calculation.}\label{fig:ff_sizes_rand_False_check_train_True_check_eval_True}
\end{figure}
...
\begin{figure}[tb!]
  ...
  \caption{Square form factors as a function of order of computation for Q-learning state selection while minimizing \(N_{\mathrm{ph}}\) during training but not during DSF calculation, for a selected DSF calculation.}\label{fig:ff_sizes_rand_False_check_train_True_check_eval_False}
\end{figure}
...
\begin{figure}[tb!]
  ...
  \caption{Square form factors as a function of order of computation for Q-learning state selection while not minimizing \(N_{\mathrm{ph}}\) during training but with minimization during DSF calculation, for a selected DSF calculation.}\label{fig:ff_sizes_rand_False_check_train_False_check_eval_True}
\end{figure}
...
\begin{figure}[tb!]
  ...
  \caption{Square form factors as a function of order of computation for Q-learning state selection without minimizing \(N_{\mathrm{ph}}\) during either training and DSF calculation, for a selected DSF calculation.}\label{fig:ff_sizes_rand_False_check_train_False_check_eval_False}
\end{figure}
...
\begin{figure}[tb!]
  ...
  \caption{Q-matrix for the Lieb-Liniger model ground state with \(N=11\) and \(I_{\max}=20\) (top). In the lower left image, the 100 lowest Q-factors are highlighted, while the 100 best are shown in the lower right image.}\label{fig:qmatrix}
\end{figure}
...
\begin{figure}[tb!]
  ...
  \caption{Squared form factor for the density operator of the Lieb-Liniger model, at \(c=1\), \(L=N=5\), \(I_{\max}=12\), calculated with ABACUS.}\label{fig:abacusformfactors_12_5}
\end{figure}
...
\begin{figure}[tb!]
  ...
  \caption{Squared form factor for the density operator of the Lieb-Liniger model, at \(c=1\), \(L=N=11\), \(I_{\max}=40\), calculated with ABACUS.}\label{fig:abacusformfactors}
\end{figure}
...
DSF scanning is an example of a hidden state task, because not all relevant information required for selecting the best action is present when only considering the current state.
...
\Cref{fig:halfcheetah} shows another aspect of performance influence by random seed.
...
Thus further research is warranted in this area, since not all models may have a system such as ABACUS in place to calculate DSFs.
...
This could provide provide insight into the working of neural networks.
